...
RendererX11::RendererX11 (unsigned int width, unsigned int height,
                          const Rgb &bg, Display *disp, int scr, Window win,
                          GC gc)
    : _disp (disp), _user_disp (false), _scr (scr), _win (win), _gc (gc),
      _user_gc (false)
{
...
void
RendererX11::draw_point (const math::Vector2 &p, const Rgb &rgb,
                         enum PointStyle s)
{
  ...
  switch (s)
    ...
    case PointStyleCross:
      ...
      XDrawLine (_disp, _win, _gc, (int)v2d.x () - t, (int)v2d.y (),
                 (int)v2d.x () + t, (int)v2d.y ());
      XDrawLine (_disp, _win, _gc, (int)v2d.x (), (int)v2d.y () - t,
                 (int)v2d.x (), (int)v2d.y () + t);
      break;

    case PointStyleRound:
      XDrawArc (_disp, _win, _gc, (int)v2d.x () - t, (int)v2d.y () - t, t * 2,
                t * 2, 0, 64 * 360);
      break;

    case PointStyleSquare:
      XDrawLine (_disp, _win, _gc, (int)v2d.x () - t, (int)v2d.y () + t,
...
void
RendererX11::draw_segment (const math::VectorPair2 &l, const Rgb &rgb)
{
  ...
  XDrawLine (_disp, _win, _gc, (int)v2da.x (), (int)v2da.y (), (int)v2db.x (),
             (int)v2db.y ());
}

void
RendererX11::draw_circle (const math::Vector2 &c, double r, const Rgb &rgb,
                          bool filled)
{
  ...
  if (filled)
    XFillArc (_disp, _win, _gc, (int)v2d.x () - xr, (int)v2d.y () - yr, xr * 2,
              yr * 2, 0, 64 * 360);
  else
    XDrawArc (_disp, _win, _gc, (int)v2d.x () - xr, (int)v2d.y () - yr, xr * 2,
              yr * 2, 0, 64 * 360);
}

void
RendererX11::draw_polygon (const math::Vector2 *array, unsigned int count,
                           const Rgb &rgb, bool filled, bool closed)
{
  ...
  if (filled)
  ...
  else
    {
      ...
      if (closed)
        XDrawLine (_disp, _win, _gc, pts[count - 1].x, pts[count - 1].y,
                   pts[0].x, pts[0].y);
    }
}

void
RendererX11::draw_text (const math::Vector2 &c, const math::Vector2 &dir,
...
long
RendererX11::get_color (const Rgb &rgb) const
{
