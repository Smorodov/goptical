...
Base::Base () : _temperature (20.0)
{
  std::cout << "material created" << std::endl;
}

Base::Base (const std::string &name_) : _temperature (20.0), name (name_)
{
...
double
Base::get_extinction_coef (double wavelen) const
{
  // Beer-Lambert law
...
double
Base::get_internal_transmittance (double wavelen, double thickness) const
{
  // Beer-Lambert law

  // FIXME simplify and check
  double tr
      = exp (-(4 * M_PI * get_extinction_coef (wavelen) * 0.001 /* 1 mm */)
             / (wavelen * 1e-9f));
...
// compute internal transmittance from extinction coefficient
double
Base::get_internal_transmittance (double wavelen) const
{
...
// material
double
Base::get_normal_reflectance (const Base *from, double wavelen) const
{
...
double
Base::get_normal_transmittance (const Base *from, double wavelen) const
{
...
void
Base::set_temperature (double temp)
{
  _temperature = temp;
}

double
Base::get_temperature () const
{
  return _temperature;
}
