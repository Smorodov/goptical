...
namespace goptical {
  namespace io {
    ...
    public:
      ...
      const std::string &get_value (unsigned scenario) const
      {
	...
	return i >= 0 && i < data_.size () ? data_[i] : 0.0;
      ...
      int get_surface_number () const { return surface_number_; }
      void dump (FILE *fp)
      {
	...
	refractive_index_ = refractive_index;
      ...
      std::shared_ptr<AsphericalData> get_aspherical_data () const
      {
	...
	aspherical_data_ = aspherical_data;
      ...
      bool is_cover_glass () const { return is_cover_glass_; }
	...
	fprintf (fp,
		 ...
		 get_thickness (scenario), diameter_, refractive_index_,
		 abbe_vd_);
      }
    ...
    private:
      void parse_thickness (const char *value,
    ...
    static bool parse_delimited (char *input_start, size_t input_size,
				 std::vector<const char *> &out_tokens,
				 char *buf, const char *delimiters) noexcept
    {
      out_tokens.clear ();
    ...
    {
      int section = 0;
      for (int i = 0; i < sizeof g_SectionMappings / sizeof (SectionMapping);
	   i++)
	{
    ...
    bool LensSpecifications::parse_file (const std::string &file_name)
    {
      ...
      while (fgets (line, sizeof line, fp) != NULL)
	{
	  ...
	  switch (current_section)
		...
		if (words.size () >= 4 && strlen (words[3]) > 0)
		  {
		    surface_data->set_refractive_index (
		      strtod (words[3], NULL));
		  }
		/* diameter */
    ...
    const DescriptiveData &LensSpecifications::get_descriptive_data () const
    {
    ...
    double add_surface (sys::Lens &lens, const Surface &surface,
			unsigned scenario = 0)
    {
	  ...
	  if (refractive_index != 0.0)
	      ...
	      if (abbe_vd == 0.0)
		{
	      ...
	      lens.add_surface (radius, aperture_radius, thickness,
				ref<material::AbbeVd>::create (refractive_index,
							       abbe_vd));
	    }
	  else
	    {
	...
	{
	  lens.add_surface (
	    ref<curve::Asphere>::create (radius, k, a4, a6, a8, a10, a12, a14),
	    ref<shape::Disk>::create (aperture_radius), thickness,
	    ref<material::AbbeVd>::create (refractive_index, abbe_vd));
	}
      else
	{
	  lens.add_surface (ref<curve::Asphere>::create (radius, k, a4, a6, a8,
							 a10, a12, a14),
			    ref<shape::Disk>::create (aperture_radius),
			    thickness);
	}
      return thickness;
    }
