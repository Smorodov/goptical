...
material::Catalog &
ImportOslo::import_catalog (const std::string &filename)
{
  ...
  std::shared_ptr<material::Catalog> cat
      = std::make_shared<material::Catalog> (filename);
  ...
  while (std::getline (file, line))
    {
      ...
      if (sscanf (buf,
                  "%15s %*s %*s %lf %lf %lf %lf %*s %*s %*s %*s %*s %u %u%n",
                  name, &density, &dndt, &tce, &transmit, &formula, &coeffcnt,
                  &len)
          < 7)
        continue;
      buf += len;
      ...
      switch (formula)
        {
        case 1:
          mat = GOPTICAL_REFNEW (material::Schott, coef[0], coef[1], coef[2],
                                 coef[3], coef[4], coef[5]);
          break;

        case 2:
          mat = GOPTICAL_REFNEW (material::Sellmeier, coef[0], coef[3],
                                 coef[1], coef[4], coef[2], coef[5]);
          break;
      ...
      switch (formula)
        {
        case 1:
          mat->set_temperature_schott (coef[0], coef[1], coef[2], coef[3],
                                       coef[4], coef[5]);
          break;

        case 2:
          mat->set_temperature_dndt (coef[0] * 1e-6f
                                     / 1000.0 /* FIXME check this */);
          break;
