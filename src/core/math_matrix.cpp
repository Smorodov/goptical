  ...
  switch (N)
    ...
    case 3:
      return _val[0][0] * (_val[1][1] * _val[2][2] - _val[2][1] * _val[1][2])
             ...
             + _val[0][2]
                   * (_val[1][0] * _val[2][1] - _val[2][0] * _val[1][1]);
    }
}

template <int N>
void
Matrix<N>::transpose (Matrix &r) const
{
  for (int i = 0; i < N - 1; i++)
    for (int j = i + 1; j < N; j++)
      {
...
template <int N>
void
Matrix<N>::inverse (Matrix &r) const
{
  assert (N <= 3 && N > 1);
    ...
    case 3:
      {
        ...
        r._val[0][0] = +s1 / det;
        r._val[0][1]
            = -(_val[0][1] * _val[2][2] - _val[0][2] * _val[2][1]) / det;
        r._val[0][2]
            = +(_val[0][1] * _val[1][2] - _val[0][2] * _val[1][1]) / det;

        r._val[1][0] = -s2 / det;
        r._val[1][1]
            = +(_val[0][0] * _val[2][2] - _val[0][2] * _val[2][0]) / det;
        r._val[1][2]
            = -(_val[0][0] * _val[1][2] - _val[0][2] * _val[1][0]) / det;

        r._val[2][0] = +s3 / det;
        r._val[2][1]
            = -(_val[0][0] * _val[2][1] - _val[0][1] * _val[2][0]) / det;
        r._val[2][2]
            = +(_val[0][0] * _val[1][1] - _val[0][1] * _val[1][0]) / det;
        break;
      }
    }
}
...
template <int N>
void
Matrix<N>::adjugate (Matrix &r) const
{
  assert (N <= 3 && N > 1);
...
template <int N>
std::ostream &
operator<< (std::ostream &o, const Matrix<N> &m)
{
  o << "[";
