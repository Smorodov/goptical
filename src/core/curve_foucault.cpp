...
Foucault::Foucault (double roc) : curveRoc (roc), _reading (), _sagitta ()
{
  init ();
}

Foucault::~Foucault () { gsl_odeiv_step_free (gsl_st); }

void
Foucault::fit (const Rotational &c)
{
  _offset = 0;
  _moving_source = true;
...
void
Foucault::add_reading (double zone_radius, double knife_offset)
{
  _updated = false;
...
unsigned int
Foucault::add_uniform_zones (double hole_radius, unsigned int count = 0)
{
...
unsigned int
Foucault::add_couder_zones (double hole_radius, unsigned int count,
  ...
  if (count == 0)
    {
      ...
      count = (unsigned int)ceil (
          ((1.0 - hole_radius / _radius) * math::square (_radius * 2.0))
...
double
Foucault::sagitta (double r) const
{
  if (!_updated)
...
double
Foucault::derivative (double r) const
{
  if (!_updated)
    const_cast<Foucault *> (this)->update ();
  ...
  while (1)
    {
      ...
      if (gsl_odeiv_step_apply (gsl_st, t, _ode_step, &y, &yerr, &dydt_in,
                                &dydt_out, &gsl_sys))
...
int
Foucault::gsl_func (double y, const double x_[], double d[], void *params)
{
