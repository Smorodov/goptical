...
struct zp_precalc_s
{
  ...
  inline zp_precalc_s (double x_, double y_)
      : x (x_), x2 (x_ * x_), y (y_), y2 (y_ * y_), r (x2 + y2)
  {
  }
};
...
static double
zp_0 (const struct zp_precalc_s &p)
{
  return 1;
}
...
static double
zp_1 (const struct zp_precalc_s &p)
{
  return p.x;
}

static void
zp_1_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 1.0;
  dxdy.y () = 0.0;
}
...
static double
zp_2 (const struct zp_precalc_s &p)
{
  return p.y;
}

static void
zp_2_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 0.0;
  dxdy.y () = 1.0;
}
...
static double
zp_3 (const struct zp_precalc_s &p)
{
  return -1.0 + 2.0 * p.r;
}

static void
zp_3_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 4.0 * p.x;
  dxdy.y () = 4.0 * p.y;
}
...
static double
zp_4 (const struct zp_precalc_s &p)
{
  return p.x2 - p.y2;
}

static void
zp_4_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 2.0 * p.x;
  dxdy.y () = -2.0 * p.y;
}
...
static double
zp_5 (const struct zp_precalc_s &p)
{
  return 2.0 * p.x * p.y;
}

static void
zp_5_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 2.0 * p.y;
  dxdy.y () = 2.0 * p.x;
}
...
static double
zp_6 (const struct zp_precalc_s &p)
{
  return p.x * (-2.0 + 3.0 * p.r);
}

static void
zp_6_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_7 (const struct zp_precalc_s &p)
{
  return p.y * (-2.0 + 3.0 * p.r);
}

static void
zp_7_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 6.0 * p.x * p.y;
...
static double
zp_8 (const struct zp_precalc_s &p)
{
  return 1.0 + 6.0 * p.r * (-1.0 + p.r);
}

static void
zp_8_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_9 (const struct zp_precalc_s &p)
{
  return p.x * (p.x2 - 3.0 * p.y2);
}

static void
zp_9_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_10 (const struct zp_precalc_s &p)
{
  return -p.y * (p.y2 - 3.0 * p.x2);
}

static void
zp_10_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 6.0 * p.x * p.y;
...
static double
zp_11 (const struct zp_precalc_s &p)
{
  return (p.y2 - p.x2) * (3.0 + (-4.0 * p.r));
}

static void
zp_11_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_12 (const struct zp_precalc_s &p)
{
  return p.x * p.y * (-6.0 + 8.0 * p.r);
}

static void
zp_12_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_13 (const struct zp_precalc_s &p)
{
  return p.x * (3.0 - p.r * (12.0 - 10.0 * p.r));
}

static void
zp_13_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_14 (const struct zp_precalc_s &p)
{
  return p.y * (3.0 - p.r * (12.0 - 10.0 * p.r));
}

static void
zp_14_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  double a = (12.0 - 10.0 * p.r);
...
static double
zp_15 (const struct zp_precalc_s &p)
{
  return -1.0 + p.r * (12 - p.r * (30.0 - 20.0 * p.r));
}

static void
zp_15_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_16 (const struct zp_precalc_s &p)
{
  return p.x2 * p.x2 + p.y2 * (p.y2 - 6.0 * p.x2);
}

static void
zp_16_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_17 (const struct zp_precalc_s &p)
{
  return 4.0 * p.x * p.y * (p.x2 - p.y2);
}

static void
zp_17_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_18 (const struct zp_precalc_s &p)
{
  return p.x * (-4.0 + 5.0 * p.r) * (p.x2 - 3.0 * p.y2);
}

static void
zp_18_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_19 (const struct zp_precalc_s &p)
{
  return p.y * (-4.0 + 5.0 * p.r) * (3.0 * p.x2 - p.y2);
}

static void
zp_19_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_20 (const struct zp_precalc_s &p)
{
  return (p.x2 - p.y2) * (6.0 - p.r * (20.0 - 15.0 * p.r));
}

static void
zp_20_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_21 (const struct zp_precalc_s &p)
{
  return p.x * p.y * (12.0 - p.r * (40.0 - 30.0 * p.r));
}

static void
zp_21_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_22 (const struct zp_precalc_s &p)
{
  return p.x * (-4.0 + p.r * (30.0 - p.r * (60.0 - 35.0 * p.r)));
}

static void
zp_22_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_23 (const struct zp_precalc_s &p)
{
  return p.y * (-4.0 + p.r * (30.0 - p.r * (60.0 - 35.0 * p.r)));
}

static void
zp_23_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  double a = 60.0 - 35.0 * p.r;
  double b = 30.0 - a * p.r;
  double c = (70.0 * p.r - 2.0 * a);
...
static double
zp_24 (const struct zp_precalc_s &p)
{
  return 1.0 - p.r * (20.0 + p.r * (-90.0 - p.r * (-140.0 + 70.0 * p.r)));
}

static void
zp_24_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_25 (const struct zp_precalc_s &p)
{
  return p.x * (p.x2 * (p.x2 - 10.0 * p.y2) + 5.0 * p.y2 * p.y2);
}

static void
zp_25_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 5.0 * math::square (p.y2) + 2.0 * math::square (p.x2)
              + 3.0 * (p.x2 - 10.0 * p.y2) * p.x2;
...
static double
zp_26 (const struct zp_precalc_s &p)
{
  return p.y * (p.y2 * (p.y2 - 10.0 * p.x2) + 5.0 * p.x2 * p.x2);
}

static void
zp_26_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = 20.0 * p.x * p.y * (p.x2 - p.y2);
  dxdy.y () = 5.0 * math::square (p.x2) + 2.0 * math::square (p.y2)
              + 3.0 * (p.y2 - 10.0 * p.x2) * p.y2;
}

// Zernike polynomial 27

static double
zp_27 (const struct zp_precalc_s &p)
{
  return (-math::square (p.x2) + p.y2 * (-p.y2 + p.x2 * 6.0))
         * (5.0 - 6.0 * p.r);
}

static void
zp_27_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x () = p.x
              * (p.y2 * (-60.0 * p.y2 - 120.0 * p.x2 + 60.0)
                 + p.x2 * (36.0 * p.x2 - 20.0));
  dxdy.y () = p.y
              * (p.x2 * (-60.0 * p.x2 - 120.0 * p.y2 + 60.0)
                 + p.y2 * (36.0 * p.y2 - 20.0));
}
...
static double
zp_28 (const struct zp_precalc_s &p)
{
  return p.x * p.y * (p.x2 - p.y2) * (-20.0 + 24.0 * p.r);
}

static void
zp_28_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  dxdy.x ()
      = p.y * (p.y2 * (-24.0 * p.y2 + 20.0) + p.x2 * (120.0 * p.x2 - 60.f));
  dxdy.y ()
      = -p.x * (p.x2 * (-24.0 * p.x2 + 20.0) + p.y2 * (120.0 * p.y2 - 60.f));
}

// Zernike polynomial 29

static double
zp_29 (const struct zp_precalc_s &p)
{
  return p.x * (10.0 - p.r * (30.0 - 21.0 * p.r)) * (p.x2 + p.y2 * -3.0);
}

static void
zp_29_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_30 (const struct zp_precalc_s &p)
{
  return -p.y * (10.0 - p.r * (30.0 - 21.0 * p.r)) * (p.y2 + p.x2 * -3.0);
}

static void
zp_30_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  double a = 30.0 - 21.0 * p.r;
  double b = 10.0 - a * p.r;
...
static double
zp_31 (const struct zp_precalc_s &p)
{
  return (p.x2 - p.y2) * (-10.0 + p.r * (60.0 - p.r * (105.0 - 56.0 * p.r)));
}

static void
zp_31_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_32 (const struct zp_precalc_s &p)
{
  return p.x * p.y * (-20.0 + p.r * (120.0 - p.r * (210.0 - (112.0 * p.r))));
}

static void
zp_32_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_33 (const struct zp_precalc_s &p)
{
  return p.x
         * (5.0 + p.r * (-60.0 + p.r * (210.0 - p.r * (280.0 - 126.0 * p.r))));
}

static void
zp_33_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
...
static double
zp_34 (const struct zp_precalc_s &p)
{
  return p.y
         * (5.0 + p.r * (-60.0 + p.r * (210.0 - p.r * (280.0 - 126.0 * p.r))));
}
...
static void
zp_34_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  double a = 280.0 - 126.0 * p.r;
  double b = 210.0 - a * p.r;
  double c = b * p.r - 60.0;
...
static double
zp_35 (const struct zp_precalc_s &p)
{
  return -1.0
         + p.r
               * (30.0
                  + p.r
                        * (-210.0
                           + p.r * (560.0 - p.r * (630.0 - 252.0 * p.r))));
}

static void
zp_35_d (const struct zp_precalc_s &p, math::Vector2 &dxdy)
{
  ...
  double e
      = 2.0
        * (p.r * (p.r * (p.r * (252.0 * p.r - a) + b) + c) + (c * p.r + 30.0));
...
zp_t *const zp[Zernike::term_count] = {
  &zp_0,  &zp_1,  &zp_2,  &zp_3,  &zp_4,  &zp_5,  &zp_6,  &zp_7,  &zp_8,
  &zp_9,  &zp_10, &zp_11, &zp_12, &zp_13, &zp_14, &zp_15, &zp_16, &zp_17,
  &zp_18, &zp_19, &zp_20, &zp_21, &zp_22, &zp_23, &zp_24, &zp_25, &zp_26,
  &zp_27, &zp_28, &zp_29, &zp_30, &zp_31, &zp_32, &zp_33, &zp_34, &zp_35
};

zp_d_t *const zp_d[Zernike::term_count]
    = { 0,        &zp_1_d,  &zp_2_d,  &zp_3_d,  &zp_4_d,  &zp_5_d,
        &zp_6_d,  &zp_7_d,  &zp_8_d,  &zp_9_d,  &zp_10_d, &zp_11_d,
        &zp_12_d, &zp_13_d, &zp_14_d, &zp_15_d, &zp_16_d, &zp_17_d,
        &zp_18_d, &zp_19_d, &zp_20_d, &zp_21_d, &zp_22_d, &zp_23_d,
        &zp_24_d, &zp_25_d, &zp_26_d, &zp_27_d, &zp_28_d, &zp_29_d,
        &zp_30_d, &zp_31_d, &zp_32_d, &zp_33_d, &zp_34_d, &zp_35_d };

Zernike::Zernike (double radius, double unit_scale)
    : _scale (unit_scale), _threshold (1e-10), _radius (radius),
      _enabled_count (0)
{
...
Zernike::Zernike (double radius, double coefs[], unsigned int coefs_count,
                  double unit_scale)
    : _scale (unit_scale), _threshold (1e-10), _radius (radius),
      _enabled_count (0)
{
...
double
Zernike::sagitta (const math::Vector2 &xy) const
{
...
void
Zernike::derivative (const math::Vector2 &xy, math::Vector2 &dxdy) const
{
  struct zp_precalc_s p (xy.x () / _radius, xy.y () / _radius);
...
double
Zernike::fit (const Base &curve, const trace::Distribution &d)
{
  ...
  shape.get_pattern (
      [&] (const math::Vector2 &item) { pattern.push_back (item); }, d, false);
  ...
  gsl_multifit_linear_workspace *work
      = gsl_multifit_linear_alloc (pcount, term_count);
...
void
Zernike::update_threshold_state ()
{
...
void
Zernike::set_term_state (unsigned int n, bool enabled)
{
  unsigned int i;
...
bool
Zernike::get_term_state (unsigned int n)
{
  for (unsigned int i = 0; i < _enabled_count; i++)
    if (_enabled_list[i] == n)
      return true;
...
double
Zernike::zernike_poly (unsigned int n, const math::Vector2 &xy)
{
...
void
Zernike::zernike_poly_d (unsigned int n, const math::Vector2 &xy,
                         math::Vector2 &dxdy)
{
  struct zp_precalc_s p (xy.x (), xy.y ());
