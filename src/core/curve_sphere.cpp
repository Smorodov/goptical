...
namespace goptical
{
	...
	namespace curve
	{
		...
		*/
		...
		bool
		Sphere::intersect (math::Vector3 &point, const math::VectorPair3 &ray) const
		{
			...
			double ay_by = ay * by;
			double ax_bx = ax * bx;
			double s = +math::square (_roc) // * bz2_by2_bx2
			           + 2.0 * (ax_bx + ay_by) * bz * d + 2.0 * ax_bx * ay_by
			           - math::square (ay * bx) - math::square (ax * by)
		...
		void
		Sphere::normal (math::Vector3 &normal, const math::Vector3 &point) const
		{
