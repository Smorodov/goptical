...
Grid::Grid (unsigned int n1, unsigned int n2, const math::Vector2 &origin,
            const math::Vector2 &step)
    : Set (), _y_data (), _d_data (), _poly (), _update (&Grid::update_linear),
      _lookup (&Grid::update_linear), _resize (&Grid::resize_y),
      _origin (origin), _step (step)
{
  _origin = origin;
  _step = step;
  resize (n1, n2);
...
void
Grid::set_all_d (const math::Vector2 &deriv)
{
  for (auto &i : _d_data)
    {
      i = deriv;
...
double
Grid::get_x_value (unsigned int x, unsigned int dimension) const
{
  assert (dimension < 2);
...
double
Grid::get_y_value (const unsigned int n[]) const
{
  return get_y_value (n[0], n[1]);
}

double
Grid::interpolate (const double x[]) const
{
  return interpolate (math::Vector2 (x[0], x[1]));
}

double
Grid::interpolate (const double x[], unsigned int deriv,
...
math::range_t
Grid::get_x_range (unsigned int d) const
{
...
void
Grid::set_interpolation (Interpolation i)
{
  switch (i)
    {
    case Nearest:
...
void
Grid::update_nearest (unsigned int x[2], const math::Vector2 &v) const
{
...
double
Grid::interpolate_nearest_y (const unsigned int x[2],
                             const math::Vector2 &v) const
{
  return _y_data[x[0] + x[1] * _size[0]];
}

void
Grid::interpolate_nearest_d (const unsigned int x[2], math::Vector2 &d,
                             const math::Vector2 &v) const
{
  d.set (0);
}
...
void
Grid::update_linear (unsigned int x[2], const math::Vector2 &v) const
{
  Grid *this_ = const_cast<Grid *> (this);
...
double
Grid::interpolate_linear_y (const unsigned int x[2],
                            const math::Vector2 &v) const
{
...
void
Grid::interpolate_linear_d (const unsigned int x[2], math::Vector2 &d,
                            const math::Vector2 &v) const
{
  unsigned int s = _size[0];
  unsigned int idx = x[0] + s * x[1];
...
void
Grid::set_poly (poly_t &p, const double t[16])
{
  /*
       0    1
    0 d00  d01
  ...
  */
  ...
  p.p[10] = (9.0 * t[0] + -9.0 * t[1] + 9.0 * t[3] + -9.0 * t[2] + 6.0 * t[4]
             + 3.0 * t[5] + -3.0 * t[7] + -6.0 * t[6] + 6.0 * t[8]
             + -6.0 * t[9] + -3.0 * t[11] + 3.0 * t[10] + 4.0 * t[12]
             + 2.0 * t[13] + 1.0 * t[15] + 2.0 * t[14]);
  p.p[11] = (-6.0 * t[0] + 6.0 * t[1] + -6.0 * t[3] + 6.0 * t[2] + -4.0 * t[4]
             + -2.0 * t[5] + 2.0 * t[7] + 4.0 * t[6] + -3.0 * t[8] + 3.0 * t[9]
             + 3.0 * t[11] + -3.0 * t[10] + -2.0 * t[12] + -1.0 * t[13]
             + -1.0 * t[15] + -2.0 * t[14]);

  p.p[12] = 2.0 * t[0] + -2.0 * t[1] + 1.0 * t[4] + 1.0 * t[5];
  p.p[13] = 2.0 * t[8] + -2.0 * t[9] + 1.0 * t[12] + 1.0 * t[13];
  p.p[14] = (-6.0 * t[0] + 6.0 * t[1] + -6.0 * t[3] + 6.0 * t[2] + -3.0 * t[4]
             + -3.0 * t[5] + 3.0 * t[7] + 3.0 * t[6] + -4.0 * t[8] + 4.0 * t[9]
             + 2.0 * t[11] + -2.0 * t[10] + -2.0 * t[12] + -2.0 * t[13]
             + -1.0 * t[15] + -1.0 * t[14]);
  p.p[15] = (4.0 * t[0] + -4.0 * t[1] + 4.0 * t[3] + -4.0 * t[2] + 2.0 * t[4]
             + 2.0 * t[5] + -2.0 * t[7] + -2.0 * t[6] + 2.0 * t[8]
             + -2.0 * t[9] + -2.0 * t[11] + 2.0 * t[10] + 1.0 * t[12]
             + 1.0 * t[13] + 1.0 * t[15] + 1.0 * t[14]);
}

void
Grid::get_cross_deriv_diff (double cd[]) const
{
  unsigned int x0, x1;
    ...
    for (x1 = 1; x1 < _size[1] - 1; x1++)
      {
        ...
        cd[idx] = (_y_data[idx + w + 1] + _y_data[idx - w - 1]
                   - _y_data[idx + w - 1] - _y_data[idx - w + 1])
                  / 4.0;
      }
...
void
Grid::get_deriv_diff (math::Vector2 d[]) const
{
  unsigned int x0, x1;
...
template <int v>
void
Grid::get_deriv_smooth (math::Vector2 d[], unsigned int w,
                        unsigned int o) const
{
  ...
  for (i = 0; i < (int)n - 1; i++)
    {
      j = o + i * w;
      d[j][v] = (_y_data[j + w] - _y_data[j])
                - (dd[i + 1] / 6.0 + dd[i] / 3.0) * (dt * dt);
    }

  i--;

  d[j + w][v] = +(_y_data[j + w] - _y_data[j])
                + (dt * dt)
                      * ((dd[i] + dt * (0.5 * (dd[i + 1] - dd[i]) / dt))
                         - (dd[i + 1] / 6.0 + dd[i] / 3.0));
...
void
Grid::update_bicubic (unsigned int x[2], const math::Vector2 &v) const
{
  Grid *this_ = const_cast<Grid *> (this);
...
void
Grid::update_bicubic_diff (unsigned int x[2], const math::Vector2 &v) const
{
  Grid *this_ = const_cast<Grid *> (this);
...
void
Grid::update_bicubic_deriv (unsigned int x[2], const math::Vector2 &v) const
{
  Grid *this_ = const_cast<Grid *> (this);
...
double
Grid::interpolate_bicubic_y (const unsigned int x[2],
                             const math::Vector2 &v) const
{
  const poly_t &p = _poly[x[0] + x[1] * (_size[0] - 1)];
  math::Vector2 t ((v - _origin) / _step
                   - math::Vector2 ((double)x[0], (double)x[1]));

  double res;

  res = ((p.p[15] * t.y () + p.p[14]) * t.y () + p.p[13]) * t.y () + p.p[12];
  res = ((p.p[11] * t.y () + p.p[10]) * t.y () + p.p[9]) * t.y () + p.p[8]
        + t.x () * res;
  res = ((p.p[7] * t.y () + p.p[6]) * t.y () + p.p[5]) * t.y () + p.p[4]
        + t.x () * res;
  res = ((p.p[3] * t.y () + p.p[2]) * t.y () + p.p[1]) * t.y () + p.p[0]
        + t.x () * res;
...
void
Grid::interpolate_bicubic_d (const unsigned int x[2], math::Vector2 &d,
                             const math::Vector2 &v) const
{
  const poly_t &p = _poly[x[0] + x[1] * (_size[0] - 1)];
  math::Vector2 t ((v - _origin) / _step
  ...
  d.x ()
      = (3.0 * p.p[12 + 3] * t.x () + 2.0 * p.p[8 + 3]) * t.x () + p.p[4 + 3];
  d.x () = (3.0 * p.p[12 + 2] * t.x () + 2.0 * p.p[8 + 2]) * t.x ()
           + p.p[4 + 2] + t.y () * d.x ();
  d.x () = (3.0 * p.p[12 + 1] * t.x () + 2.0 * p.p[8 + 1]) * t.x ()
           + p.p[4 + 1] + t.y () * d.x ();
  d.x () = (3.0 * p.p[12 + 0] * t.x () + 2.0 * p.p[8 + 0]) * t.x ()
           + p.p[4 + 0] + t.y () * d.x ();
  d.x () /= _step.x ();

  d.y () = (3.0 * p.p[12 + 3] * t.y () + 2.0 * p.p[12 + 2]) * t.y ()
           + p.p[12 + 1];
  d.y () = (3.0 * p.p[8 + 3] * t.y () + 2.0 * p.p[8 + 2]) * t.y () + p.p[8 + 1]
           + t.x () * d.y ();
  d.y () = (3.0 * p.p[4 + 3] * t.y () + 2.0 * p.p[4 + 2]) * t.y () + p.p[4 + 1]
           + t.x () * d.y ();
  d.y () = (3.0 * p.p[0 + 3] * t.y () + 2.0 * p.p[0 + 2]) * t.y () + p.p[0 + 1]
           + t.x () * d.y ();
  d.y () /= _step.y ();
}
...
void
Grid::lookup_nearest (unsigned int x[2], const math::Vector2 &v) const
{
...
void
Grid::lookup_interval (unsigned int x[2], const math::Vector2 &v) const
{
...
void
Grid::resize_y (unsigned int x1, unsigned int x2)
{
  invalidate ();
  assert (x1 >= 2 && x2 >= 2);
...
void
Grid::resize_yd (unsigned int x1, unsigned int x2)
{
  invalidate ();
  assert (x1 >= 2 && x2 >= 2);
  _y_data.resize (x1 * x2, 0.0);
