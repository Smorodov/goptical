...
RegularPolygon::RegularPolygon (double radius, unsigned int edge_cnt,
                                double angle)
    : _radius (radius), _edge_cnt (edge_cnt), _angle (math::degree2rad (angle))
{
  assert (_edge_cnt >= 3);
...
math::VectorPair2
RegularPolygon::get_edge (const math::Vector2 &point) const
{
  // find sector start angle
  double a = math::lp_floor (atan2 (point.y (), point.x ()) - _angle,
                             (double)_edge_cnt / (2.0 * M_PI));

  // find sector edge coordinates
  return math::VectorPair2 (_radius * cos (_angle + a),
                            _radius * sin (_angle + a),
                            _radius * cos (_angle + a + _a_step),
                            _radius * sin (_angle + a + _a_step));
}

bool
RegularPolygon::inside (const math::Vector2 &point) const
{
  ...
  return ((point.y () - e.y0 ()) * (e.x1 () - e.x0 ())
              - (point.x () - e.x0 ()) * (e.y1 () - e.y0 ())
          > 0);
}

math::VectorPair2
RegularPolygon::get_bounding_box () const
{
...
unsigned int
RegularPolygon::get_contour_count () const
{
  return 1;
}

double
RegularPolygon::get_radial_step (double resolution) const
{
...
void
RegularPolygon::get_contour (unsigned int contour,
...
void
RegularPolygon::get_triangles (const math::Triangle<2>::put_delegate_t &f,
                               double resolution) const
{
  static const double epsilon = 1e-8;
      ...
      for (double a1 = 0; a1 < 2.0 * M_PI - epsilon; a1 += astep1)
        {
          ...
          math::Vector2 b (cos (_angle + a1 + astep1),
                           sin (_angle + a1 + astep1));
          math::Vector2 ba (b * r);
...
double
RegularPolygon::get_outter_radius (const math::Vector2 &dir) const
{
