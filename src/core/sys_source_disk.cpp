...
SourceDisk::SourceDisk (SourceInfinityMode m, const math::Vector3 &pos_dir,
                        const math::Vector2 &size, const math::Vector2 &limit1,
    ...
    : Source (
        m == SourceAtInfinity
            ...
            //                   math::VectorPair3(pos_dir, math::vector3_001)
            : math::VectorPair3 (math::Vector3 (0, 0, 0) /*pos_dir * -1e9*/,
                                 pos_dir)),
      _mode (m), _size (size), _direction (pos_dir), _limit1 (limit1),
      _limit2 (limit2)
{
}
...
void
SourceDisk::get_lightrays_ (trace::Result &result, const Element &target) const
{
  ...
  auto de = [&] (const math::Vector3 &i) {
    // math::Vector3 r_ = starget->get_transform_to(*this).transform(i);  //
    // pattern point on target surface

    if (i[0] < _limit1[0] || i[0] > _limit2[0] || i[1] < _limit1[1]
        || i[1] > _limit2[1])
      {
        return;
      }
    ...
    switch (mode)
      ...
      case (SourceAtInfinity):
        ...
        //              exit(-1);
        //              for (double xi = -_size.x()/2.0; xi < _size.x()/2.0;
        //              xi+=_size.x()/10) {
        //                  for (double yi = -_size.y()/2.0; yi <
        //                  _size.y()/2.0; yi+=_size.y()/10) {
        for (auto &l : this->_spectrum)
          {
            ...
            r.set_creator (this);
            r.set_intensity (
                l.get_intensity ()); // FIXME depends on distance from source
                                     // and pattern density
...
void
SourceDisk::generate_rays_simple (trace::Result &result,
...
void
SourceDisk::generate_rays_intensity (trace::Result &result,
                                     const targets_t &entry) const
{
...
void
SourceDisk::set_limits (const math::Vector2 &limit1,
                        const math::Vector2 &limit2)
{
  _limit1 = limit1;
  _limit2 = limit2;
