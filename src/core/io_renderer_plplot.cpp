...
RendererPlplot::RendererPlplot (const char *device, const char *filename,
                                ...
                                const Rgb &bg)
    : _user_pls (false), _palette (), _palette_size (16)
{
  ...
  // backend init
  _pls->sdev (device);
  _pls->spage (0.0, 0.0, (PLINT)_2d_output_res.x (),
               (PLINT)_2d_output_res.y (), 0, 0);
...
RendererPlplot::RendererPlplot (plstream *pls)
    : _pls (pls), _user_pls (true), _palette (), _palette_size (16)
{
  ...
  // FIXME init _styles_color[StyleBackground] and [StyleForeground] using
  // gcolbg
...
void
RendererPlplot::set_page_layout (unsigned int cols, unsigned int rows)
{
  _cols = cols;
  _rows = rows;
  _pls->ssub (cols, rows);
...
void
RendererPlplot::set_page (unsigned int page)
{
  _pageid = page;
  _pls->adv (page + 1);
}
...
void
RendererPlplot::pl_set_drawing_window ()
{
  // margin inside drawable area
  _pls->vpor (0., 1., 0., 1.);

  _pls->wind (_window2d[0].x (), _window2d[1].x (), _window2d[0].y (),
              _window2d[1].y ());
}

void
RendererPlplot::pl_set_plot_window ()
{
  // margin outside drawable area
  _pls->vpor ((_window2d_fit[0].x () - _window2d[0].x ())
                  / (_window2d[1].x () - _window2d[0].x ()),
              1.
                  - (_window2d[1].x () - _window2d_fit[1].x ())
                        / (_window2d[1].x () - _window2d[0].x ()),
              (_window2d_fit[0].y () - _window2d[0].y ())
                  / (_window2d[1].y () - _window2d[0].y ()),
              1.
                  - (_window2d[1].y () - _window2d_fit[1].y ())
                        / (_window2d[1].y () - _window2d[0].y ()));
...
void
RendererPlplot::update_2d_window ()
{
  pl_set_drawing_window ();
}

void
RendererPlplot::set_2d_size (double width, double height)
{
...
void
RendererPlplot::draw_point (const math::Vector2 &v, const Rgb &rgb,
                            enum PointStyle s)
{
  int code;
...
void
RendererPlplot::draw_segment (const math::VectorPair2 &l, const Rgb &rgb)
{
  _pls->col0 (get_color_id (rgb));
...
void
RendererPlplot::draw_circle (const math::Vector2 &c, double r, const Rgb &rgb,
                             bool filled)
{
  _pls->col0 (get_color_id (rgb));
  std::cout << "draw_circle was not tested (added 8th parameter to compile"
            << std::endl;
  exit (-1);
...
void
RendererPlplot::draw_polygon (const math::Vector2 *array, unsigned int count,
...
void
RendererPlplot::draw_text (const math::Vector2 &c, const math::Vector2 &dir,
...
void
RendererPlplot::pl_draw_axes (const RendererAxes &a)
{
  ...
  _pls->axes (ap.x (), ap.y (), xopts.c_str (), step.x (), 0, yopts.c_str (),
              step.y (), 0);
}

void
RendererPlplot::draw_axes_2d (const RendererAxes &a)
{
  pl_set_plot_window ();
  pl_draw_axes (a);
  pl_set_drawing_window ();
...
void
RendererPlplot::draw_axes_3d (const RendererAxes &a,
                              const math::Vector3 &range)
{
  // FIXME
}

void
RendererPlplot::draw_plot (const data::Plot &plot)
{
...
int
RendererPlplot::get_color_id (const Rgb &rgb)
{
  ...
  if (i == _palette.end ())
    {
      ...
      _pls->scol0 (_palette_size, (int)(rgb.r * 255), (int)(rgb.g * 255),
                   (int)(rgb.b * 255));
