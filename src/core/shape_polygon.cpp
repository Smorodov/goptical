...
Polygon::Polygon ()
    : _updated (false), _vertices (), _bbox (math::vector2_pair_00),
      _max_radius (0), _min_radius (1e100)
{
}
  ...
  for (unsigned int i = 0; i < s; i++)
    {
      ...
      // update min radius
      double min = math::VectorPair2 (*cur, *prev)
                       .seg_pt_clst_pt (math::vector2_0)
                       .len ();
      if (min < _min_radius)
        _min_radius = min;
...
void
Polygon::insert_vertex (const math::Vector2 &v, unsigned int id)
{
  _updated = false;
...
unsigned int
Polygon::add_vertex (const math::Vector2 &v)
{
  _updated = false;
...
void
Polygon::delete_vertex (unsigned int id)
{
  _updated = false;
  assert (id < _vertices.size ());
...
bool
Polygon::inside (const math::Vector2 &p) const
{
...
void
Polygon::get_contour (unsigned int contour,
...
void
Polygon::get_triangles (const math::Triangle<2>::put_delegate_t &f,
                        double resolution) const
{
...
math::VectorPair2
Polygon::get_bounding_box () const
{
  if (!_updated)
    const_cast<Polygon *> (this)->update ();
  return _bbox;
...
double
Polygon::get_outter_radius (const math::Vector2 &dir) const
{
  if (!_updated)
    const_cast<Polygon *> (this)->update ();
  ...
  for (unsigned int i = 0; i < s; i++)
    {
      ...
      try
        {
          double a = math::VectorPair2 (*w, *v)
                         .seg_intersect_seg<false, false> (d)
                         .len ();
          r = std::max (r, a);
        }
      catch (...)
