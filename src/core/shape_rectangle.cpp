...
bool
Rectangle::inside (const math::Vector2 &point) const
{
  return (fabs (point.x ()) <= _halfsize.x ()
          && fabs (point.y ()) <= _halfsize.y ());
}

void
Rectangle::get_pattern (const math::Vector2::put_delegate_t &f,
...
unsigned int
Rectangle::get_contour_count () const
{
  return 1;
}

inline math::Vector2
Rectangle::get_step (double resolution) const
{
  math::Vector2 step;
...
void
Rectangle::get_contour (unsigned int contour,
...
void
Rectangle::get_triangles (const math::Triangle<2>::put_delegate_t &f,
                          double resolution) const
{
  static const double epsilon = 1e-8;
  math::Vector2 step = get_step (resolution);
...
double
Rectangle::get_outter_radius (const math::Vector2 &dir) const
{
  const math::Vector2 &e
      = (fabs (dir.x () / dir.y ()) < _halfsize.x () / _halfsize.y ())
            ? math::vector2_10
            : math::vector2_01;

  return (math::VectorPair2 (_halfsize, e)
              .ln_intersect_ln (math::VectorPair2 (math::vector2_0, dir)))
      .len ();
}
