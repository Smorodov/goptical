...
Stop::Stop (const math::VectorPair3 &p,
            const std::shared_ptr<shape::Base> &shape)
    : Surface (p, curve::flat, shape), _intercept_reemit (false)
{
...
bool
Stop::intersect (const trace::Params &params, math::VectorPair3 &intersect,
...
inline void
Stop::trace_ray_simple (trace::Result &result, trace::Ray &incident,
...
void
Stop::trace_ray_intensity (trace::Result &result, trace::Ray &incident,
                           const math::VectorPair3 &local,
                           const math::VectorPair3 &intersect) const
{
...
inline void
Stop::process_rays_ (trace::Result &result, trace::rays_queue_t *input) const
{
  for (auto &i : *input)
    {
...
void
Stop::process_rays_simple (trace::Result &result,
...
void
Stop::process_rays_intensity (trace::Result &result,
                              trace::rays_queue_t *input) const
{
...
void
Stop::process_rays_polarized (trace::Result &result,
                              trace::rays_queue_t *input) const
{
...
void
Stop::draw_3d_e (io::Renderer &r, const Element *ref) const
{
  r.group_begin ();
...
void
Stop::draw_2d_e (io::Renderer &r, const Element *ref) const
{
  math::Vector3 mr (0, _external_radius, 0);
  math::Vector3 top (0, get_shape ().get_outter_radius (math::vector2_01), 0);
  math::Vector3 bot (0, -get_shape ().get_outter_radius (-math::vector2_01),
                     0);
