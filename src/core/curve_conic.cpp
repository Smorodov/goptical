...
Conic::Conic (double roc, double sc) : ConicBase (roc, sc) {}

inline double
Conic::derivative (double r) const
{
  ...
  const double s4
      = 2.0 / (_roc * (s3 + 1))
        + s2 / (math::square (_roc) * _roc * s3 * math::square (s3 + 1));

  return r * s4;
}

double
Conic::sagitta (double r) const
{
  return math::square (r)
         / (_roc
            * (sqrt (1 - (_sh * math::square (r)) / math::square (_roc)) + 1));
}

bool
Conic::intersect (math::Vector3 &point, const math::VectorPair3 &ray) const
{
  ...
  */
  ...
  double c = (_sh * math::square (az) + math::square (ay) + math::square (ax))
                 / _roc
             - 2.0 * az;
...
double
Conic::fit (const Rotational &c, double radius, unsigned int count)
{
  ...
  gsl_fit_linear (X, 1, Y, 1, count, &c0, &c1, &cov00, &cov01, &cov11, &chisq);
