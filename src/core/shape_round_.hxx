...
void
Round<X, hole>::get_pattern (const math::Vector2::put_delegate_t &f,
...
template <class X, bool hole>
unsigned int
Round<X, hole>::get_contour_count () const
{
  return hole ? 2 : 1;
}
...
template <class X, bool hole>
inline double
Round<X, hole>::get_radial_step (double resolution) const
{
  const double xyr = 1.0 / X::get_xy_ratio ();
  double width = xyr <= 1.
                     ? X::get_external_xradius () - X::get_internal_xradius ()
                     : X::get_external_xradius () * xyr
                           - X::get_internal_xradius () * xyr;
...
template <class X, bool hole>
void
Round<X, hole>::get_contour (unsigned int contour,
