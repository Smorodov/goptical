...
void
RendererViewport::set_2d_size (double width, double height)
{
  _2d_output_res = math::Vector2 (width, height);
}

void
RendererViewport::set_window (const math::Vector2 &center,
                              const math::Vector2 &size, bool keep_aspect)
{
  math::Vector2 s (size);
...
void
RendererViewport::update_2d_window ()
{
}

void
RendererViewport::set_window (const math::Vector2 &center, double radius,
                              bool keep_aspect)
{
...
void
RendererViewport::set_window (const math::VectorPair2 &window,
                              bool keep_aspect)
{
...
void
RendererViewport::set_page_layout (unsigned int cols, unsigned int rows)
{
  _cols = cols;
  _rows = rows;
  set_page (0);
...
void
RendererViewport::set_page (unsigned int page)
{
  if (page >= _cols * _rows)
  ...
  math::Vector2 b (a.x () + size.x () * _cols, a.y () + size.y () * _rows);

  _page = math::VectorPair2 (a, b);
}

void
RendererViewport::draw_pages_grid (const Rgb &rgb)
{
  math::Vector2 size (_window2d[1].x () - _window2d[0].x (),
                      _window2d[1].y () - _window2d[0].y ());
    ...
    for (unsigned int y = 0; y < _rows; y++)
      {
        if (y)
          draw_segment (
              math::VectorPair2 (_page.x0 (), _page.y0 () + size.y () * y,
                                 _page.x1 (), _page.y0 () + size.y () * y));

        if (x)
          draw_segment (
              math::VectorPair2 (_page.x0 () + size.x () * x, _page.y0 (),
                                 _page.x0 () + size.x () * x, _page.y1 ()));
      }
}

void
RendererViewport::set_margin (double left, double bottom, double right,
                              double top)
{
...
void
RendererViewport::set_margin_ratio (double left, double bottom, double right,
                                    double top)
{
  _margin_type = MarginRatio;
  _margin = math::VectorPair2 (left, bottom, right, top);
...
void
RendererViewport::set_margin_output (double left, double bottom, double right,
                                     double top)
{
...
void
RendererViewport::set_camera_direction (const math::Vector3 &dir)
{
...
void
RendererViewport::set_camera_position (const math::Vector3 &pos)
{
  math::Transform<3> t (get_camera_transform ());
...
void
RendererViewport::clear ()
{
}

void
RendererViewport::flush ()
{
}
...
void
RendererViewport::set_2d_plot_window (const data::Plot &plot)
{
  ...
  set_window (
      math::VectorPair2 (math::Vector2 (x_range.first, y_range.first),
                         math::Vector2 (x_range.second, y_range.second)),
      false);
}

void
RendererViewport::draw_plot (const data::Plot &plot)
{
  switch (plot.get_dimensions ())
    ...
    case 1:
      ...
      // plot title
      draw_text (
          math::Vector2 ((_window2d[0].x () + _window2d[1].x ()) / 2.,
                         (_window2d_fit[1].y () + _window2d[1].y ()) / 2.),
          math::vector2_10, plot.get_title (),
          TextAlignCenter | TextAlignMiddle, 18,
      ...
      for (unsigned int i = 0; i < plot.get_plot_count (); i++)
        {
          ...
          draw_plot_data_2d (static_cast<const data::Set1d &> (d.get_set ()),
                             d);
        }
...
void
RendererViewport::draw_frame_2d ()
{
  math::Vector2 fr[4];
...
void
RendererViewport::draw_plot_data_2d (const data::Set1d &data,
                                     const data::Plotdata &style)
{
  ...
  if (style.get_style () & data::InterpolatePlot)
    {
      const double x_step
          = (_window2d[1].x () - _window2d[0].x ()) / _2d_output_res.x ();
  ...
  if (style.get_style () & data::LinePlot)
    {
      std::pair<double, double> p1 (data.get_x_value ((unsigned)0),
                                    data.get_y_value ((unsigned)0));

      for (unsigned int j = 1; j < data.get_count (); j++)
        {
          std::pair<double, double> p2 (data.get_x_value (j),
                                        data.get_y_value (j));

          draw_segment (
              math::VectorPair3 (math::Vector3 (p1.first, p1.second, 0),
      ...
      for (unsigned int j = 0; j < data.get_count (); j++)
        {
          ...
          std::pair<double, double> p (data.get_x_value (j),
                                       data.get_y_value (j));
          ...
          draw_text (math::Vector2 (p.first, p.second), math::vector2_10,
                     s.str (), (TextAlignMask)a, 12, style.get_color ());
        }
    }
}

void
RendererViewport::draw_plot_data_3d (const data::Grid &data,
                                     const data::Plotdata &style)
{
  ...
  if (style.get_style () & data::Filled)
    {
      ...
      for (double x = xr.first; x < xr.second - epsilon; x += xs)
        for (double y = yr.first; y < yr.second - epsilon; y += ys)
          {
            math::Vector3 g11 (data.interpolate_deriv (math::Vector2 (x, y)),
                               -1);
            math::Vector3 g12 (
                data.interpolate_deriv (math::Vector2 (x, y + ys)), -1);
            math::Vector3 g21 (
                data.interpolate_deriv (math::Vector2 (x + xs, y)), -1);
            math::Vector3 g22 (
                data.interpolate_deriv (math::Vector2 (x + xs, y + ys)), -1);
...
#endif
            draw_triangle (math::Triangle<3> (p11, p12, p22),
                           math::Triangle<3> (g11, g12, g22), Rgb (0, 1, 0));
            draw_triangle (math::Triangle<3> (p22, p21, p11),
                           math::Triangle<3> (g22, g21, g11), Rgb (0, 1, 0));
          }
    }
  ...
  if (style.get_style () & data::LinePlot)
    {
      ...
      for (x = 0; x < data.get_count (0); x++)
        {
          math::Vector3 p1 (data.get_x_value (x, 0), data.get_x_value (0, 1),
      ...
      for (y = 0; y < data.get_count (0); y++)
        {
          math::Vector3 p1 (data.get_x_value (0, 0), data.get_x_value (y, 1),
  ...
  if (style.get_style () & data::PointPlot)
    {
      for (unsigned int x = 0; x < data.get_count (0); x++)
        for (unsigned int y = 0; y < data.get_count (1); y++)
          {
            math::Vector3 p (data.get_x_value (x, 0), data.get_x_value (y, 1),
                             data.get_y_value (x, y));
...
void
RendererViewport::draw_axes_2d (const RendererAxes &a)
{
  draw_axes<2> (a);
}

void
RendererViewport::draw_axes_3d (const RendererAxes &a)
{
  //      draw_axes<3>(a, range);
}
...
template <int N>
void
RendererViewport::draw_axes (const RendererAxes &a)
{
  ...
  for (unsigned int i = 0; i < N; i++)
    {
      ...
      switch (i)
        ...
        case 0:
          lp = math::Vector2 (
              (_window2d[0].x () + _window2d[1].x ()) / 2.,
              (_window2d_fit[0].y () * .50 + _window2d[0].y () * 1.50) / 2.);
          ld = math::vector2_10;
          break;
        case 1:
          lp = math::Vector2 (
              (_window2d_fit[0].x () * .50 + _window2d[0].x () * 1.50) / 2.,
      ...
      // axis label
      {
        ...
        draw_text (lp, ld, lx, TextAlignCenter | TextAlignMiddle, 12,
                   get_style_color (StyleForeground));
      }
      ...
      for (unsigned int j = 0; j < N; j++)
        oor |= (j != i
                && ((p[j]
                     <= std::min (_window2d_fit[0][j], _window2d_fit[1][j]))
                    || (p[j] >= std::max (_window2d_fit[0][j],
                                          _window2d_fit[1][j]))));
            ...
            switch (N)
              ...
              case 3:
                for (int z = min[2]; z <= max[2]; z++)
                  draw_point (math::Vector3 (p[0] + x * step[0],
                                             p[1] + y * step[1],
                                             p[2] + z * step[2]),
                              get_style_color (StyleForeground));
                break;
              ...
              case 2:
                draw_point (
                    math::Vector2 (p[0] + x * step[0], p[1] + y * step[1]),
                    get_style_color (StyleForeground));
                break;
              }
          }
...
template <int N>
void
RendererViewport::draw_axes_tic (const RendererAxes &a, unsigned int i,
                                 int pow10, bool oor, double x)
{
  ...
  if (ax._values)
	...
	// Following doesn't seem to work on Windows 10
      ...
      char temp[80];
      snprintf (temp, sizeof temp, "%.3g",
                (x + p[i] - a._origin[i]) / pow (10., pow10));
      std::string s (temp);
#endif
      ...
      switch (N)
        ...
        case 3:
          ...
          break;
        case 2:
          draw_text (vtic, math::vector2_10, s, align[i], 12,
                     get_style_color (StyleForeground));
          break;
        }
    }
