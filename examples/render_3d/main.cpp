...
void main(int argc, char** argv)
{
	...
	// anchor end 
	//--------------------------
	// Optical opt_system definition
	// anchor lens_shape 
	auto lens_shape = std::make_shared<shape::Disk> (100); // lens diameter is 100mm
	...
	// anchor image 
	// image plane
	auto image = std::make_shared<sys::Image> (math::Vector3 (0, 0, 16.35), // position
	                                    10); // square size,
	// anchor opt_sys 
	//opt_sys = std::make_shared<sys::System> ();
	...
	lens->add_surface (-20, 5.0, 3, std::make_shared<material::AbbeVd> (1.607170, 59.5002));
	lens->add_surface (20, 5.0, 0);
...
// anchor seq 
    ...
    // fill arrays  
    renderer = (io::Renderer*) new io::RendererOpengl(20., 100000., 640, 480);
    auto ctx=static_cast<io::Context*>(renderer->user_data);
    ctx->clear();  
