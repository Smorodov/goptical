...
void
Dielectric::clear_internal_transmittance ()
{
  _transmittance.clear ();
}

data::DiscreteSet &
Dielectric::get_transmittance_dataset ()
{
  return _transmittance;
}

void
Dielectric::set_temperature_schott (double d0, double d1, double d2, double e0,
...
void
Dielectric::set_temperature_dndt (double dndt)
{
...
void
Dielectric::disable_temperature_coeff ()
{
  _temp_model = ThermalNone;
}

void
Dielectric::set_measurement_medium (const std::shared_ptr<Base> &medium)
{
...
void
Dielectric::set_wavelen_range (double low, double high)
{
  _low_wavelen = low;
  _high_wavelen = high;
}
