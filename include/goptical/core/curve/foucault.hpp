...
public:
  ...
  */
  ...
  /** Clear current zones and add zones readings to locations
      ...
      @return Zone count
  */
  unsigned int add_couder_zones (double hole_radius, unsigned int count = 0,
                                 std::vector<double> *edge = 0);
  ...
  */
  ...
  get_reading (unsigned int zone_number) const;
  ...
  /** Set moving light source foucault test. This clear all
      readings and zones.
      @param source_offset Distance between knife edge and light source along
     optical axis. */
  inline void set_moving_source (double source_offset = 0.0);

  /** Set fixed light source foucault test. This clear all
      readings and zones.
      @param source_to_surface Distance between surface and light source along
     the optical axis. */
...
void
Foucault::set_moving_source (double offset)
{
  _updated = false;
  _moving_source = true;
  _offset = offset;
...
void
Foucault::set_fixed_source (double source_to_surface)
{
  _updated = false;
...
void
Foucault::set_radius (double radius)
{
  _updated = false;
  _radius = radius;
}
...
double
Foucault::get_radius () const
{
  return _radius;
}

void
Foucault::set_ode_stepsize (double step)
{
  _updated = false;
  _ode_step = step;
}
...
unsigned int
Foucault::get_zones_count () const
{
  return _reading.get_count ();
}

void
Foucault::set_knife_offset (unsigned int zone_number, double knife_offset)
{
  _reading.get_y_value (zone_number) = knife_offset;
}

const std::pair<double, double>
Foucault::get_reading (unsigned int index) const
{
