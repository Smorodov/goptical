...
private:
  ...
  static void set_linear_poly (struct poly_s &p, double p1x, double p1y,
                               double p2x, double p2y);

  /** set polynomial coefficients of line through 1 points with given
   * derivative */
  static void set_linear_poly (struct poly_s &p, double p1x, double p1y,
                               double d1);

  /** set polynomial coefficients of parabola through 3 points */
  static void set_quadratic_poly (struct poly_s &p, double p1x, double p1y,
  ...
  static void set_quadratic_poly (struct poly_s &p, double px, double py,
                                  double d, double dd);

  /** set polynomial coefficients of cubic through 2 points with
      given first derivatives */
  static void set_cubic_poly (struct poly_s &p, double p1x, double p1y,
                              double p2x, double p2y, double d1, double d2);

  /** set polynomial coefficients of cubic through 2 points with
      given second derivatives */
  static void set_cubic_poly2 (struct poly_s &p, double p1x, double p1y,
                               double p2x, double p2y, double dd1, double dd2);

  /** compute cubic sencond derivatives using the tridiagonal algorithm */
  template <cubic_2nd_deriv_init_e de>
  void compute_cubic_2nd_deriv (unsigned int n, double dd[], double d0,
                                double dn) const;
...
template <class X>
double
Interpolate1d<X>::interpolate (double x) const
{
...
template <class X>
double
Interpolate1d<X>::interpolate (double x, unsigned int d) const
{
