...
public:
  ...
  inline void set_numeric_format (std::ios_base::fmtflags format,
  ...
  virtual void set_window (const math::Vector2 &center,
                           const math::Vector2 &size, bool keep_aspect = true);
  void set_window (const math::VectorPair2 &window, bool keep_aspect = true);
  void set_window (const math::Vector2 &center, double radius,
  ...
  /** @internal Draw scale, axis, ruler tics. Current 2d window is used as
   * range */
...
protected:
  ...
  virtual void draw_plot_data_2d (const data::Set1d &data,
                                  const data::Plotdata &plot);
  /** @internal Draw plot data */
  virtual void draw_plot_data_3d (const data::Grid &data,
                                  const data::Plotdata &plot);
...
private:
  template <int N> void draw_axes (const RendererAxes &a);

  template <int N>
  void draw_axes_tic (const RendererAxes &a, unsigned int i, int pow10,
                      bool oor, double x);
...
void
RendererViewport::set_numeric_format (std::ios_base::fmtflags format,
                                      unsigned int precision)
{
...
const math::VectorPair2 &
RendererViewport::get_window () const
{
  return _window2d_fit;
}

double
RendererViewport::x_scale (double x) const
{
  return ((x / (_page[1].x () - _page[0].x ())) * _2d_output_res.x ());
}

double
RendererViewport::y_scale (double y) const
{
  return ((y / (_page[1].y () - _page[0].y ())) * _2d_output_res.y ());
}

double
RendererViewport::x_trans_pos (double x) const
{
  return x_scale (x - _page[0].x ());
}

double
RendererViewport::y_trans_pos (double y) const
{
  return y_scale (y - _page[0].y ());
}

void
RendererViewport::set_margin_output (double width, double height)
{
  set_margin_output (width, height, width, height);
}

void
RendererViewport::set_margin (double width, double height)
{
  set_margin (width, height, width, height);
}

void
RendererViewport::set_margin_ratio (double width, double height)
{
