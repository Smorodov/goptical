...
private:
  ...
  /** @override */
  void draw_circle (const math::Vector2 &c, double r, const Rgb &rgb,
                    bool filled);
  /** @override */
  ...
  /** @override */
  void draw_plot_data_2d (const data::Set1d &data,
  ...
  void svg_begin_ellipse (double x, double y, double rx, double ry,
                          bool terminate = false);
  void svg_begin_use (const std::string &id, double x, double y,
                      bool terminate = false);
...
math::Vector2
RendererSvg::trans_pos (const math::Vector2 &v)
{
  return math::Vector2 (x_trans_pos (v.x ()), y_trans_pos (v.y ()));
}

double
RendererSvg::y_trans_pos (double y) const
{
  return (((y - _page[1].y ()) / (_page[0].y () - _page[1].y ()))
          * _2d_output_res.y ());
}
}
}
