  ...
  namespace io {
    ...
    public:
      ...
      /** Import Zemax ascii glass catalog file (@tt .agf). Guess catalog
          name from file name */
      std::shared_ptr<material::Catalog> import_catalog_file(const std::string &path);
      ...
      std::shared_ptr<material::Dielectric> import_table_glass(const std::string &filename);

    private:

      static std::string basename(const std::string &path);

      std::shared_ptr<shape::Base> get_ap_shape(const struct zemax_surface_s &surf, double unit_factor) const;
      std::shared_ptr<material::Base> get_glass(sys::System &sys, const struct zemax_surface_s &surf) const;
