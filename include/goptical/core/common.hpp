...
#define GOPTICAL_SET_ACCESSOR(type, name, help)                               \
  ...
  inline void set_##name (type name) { _##name = name; }

/** @internal @showcontent */
#define GOPTICAL_GET_ACCESSOR(type, name, help)                               \
  /** @see set_##name Get ""help"". */                                        \
  inline type get_##name () const { return _##name; }
...
#define GOPTICAL_SET_REF_ACCESSOR(type, name, help)                           \
  /** @see get_##name Set ""help"". */                                        \
  inline void set_##name (const type &name) { _##name = name; }
...
/** Specifies point distribution patterns over a shape delimited surface.
    @image dist_patterns.png {Different patterns rendered on a disk with
   default density}
    @see Distribution
*/
enum Pattern
...
static inline double
lp_floor (double x, double n)
{
  return floor (x * n) / n;
}

static inline double
lp_mod (double x, double n)
{
...
/** @see SellmeierMod
    @short Handbook of optics variation of the Sellmeier model for optical
   glass material */
