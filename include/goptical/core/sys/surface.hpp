...
public:
  ...
  */
  virtual bool intersect (const trace::Params &params, math::VectorPair3 &pt,
...
private:
  /** @internal */
  void get_2d_points (math::Vector2 *array, unsigned int count, double start,
...
void
Surface::set_curve (const std::shared_ptr<curve::Base> &c)
{
  _curve = c;
}

const curve::Base &
Surface::get_curve () const
{
  return *_curve;
}

void
Surface::set_shape (const std::shared_ptr<shape::Base> &s)
{
  _shape = s;
}
...
void
Surface::set_discard_intensity (double intensity)
{
  _discard_intensity = intensity;
}

inline double
Surface::get_discard_intensity () const
{
